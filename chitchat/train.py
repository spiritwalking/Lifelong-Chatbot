...
def set_args():
    ...
    parser.add_argument('--device_ids', default='2,3', type=str, help='设置使用哪些显卡')
    ...
    parser.add_argument('--train_path', default='data/my_train.pkl', type=str, help='训练集路径')
    ...
    parser.add_argument('--epochs', default=40, type=int, help='训练的最大轮次')
    ...
    parser.add_argument('--pretrained_model', default='', type=str, help='预训练的模型的路径')
...
def train_epoch(model, train_dataloader, optimizer, scheduler, logger, epoch, args):
    ...
    for batch_idx, (input_ids, labels) in enumerate(train_dataloader):
        ...
        try:
            ...
            # 这里不用输入加入attention_mask，因为GPT做的都是masked self attention
            # 所以会自动将所有future token给mask掉，包括所有的pad token。
